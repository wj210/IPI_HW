...
import re
from utils.utils import aside_encode,get_aside_segments,aside_encode_start
...
from aside_dpo import DPOTrainerWithSegments,apply_chat_tool_template
...
class AlpacaDataset(Dataset):
    ...
    def __init__(self, data: List[Dict], template_info: Dict, handler: CustomModelHandler, max_length=512,combine_system_user = True, start_token=None, end_token=None): # combine system user means we leave the system prompt out and put everything inside the user prompt.
        self.data = data
    ...
    def __getitem__(self, idx):
        ...
        if not self.combine_system_user:
            ...
            if self.handler.embedding_type  == 'forward_rot':
                ...
                encoded = aside_encode_start(encoded,self.handler.tokenizer,self.start_token)
...
class ToolDataset(Dataset):
    ...
    def __init__(self, data: List[Dict],handler: CustomModelHandler, assistant_token, tool_token, max_length=2048,last_completion_only=False):
        self.data = data
        self.handler = handler
        self.max_length = max_length
        self.printed_input = False
    ...
    def __getitem__(self, idx):
        ...
        # else:
        conv = []
        key_name = 'from' if 'from' in data_point['conversations'][0] else 'role'
        value_name = 'value' if 'value' in data_point['conversations'][0] else 'content'
        for x in data_point['conversations']:
            if x[key_name] == 'function': # convert to tool
                x[key_name] = 'tool'
            conv.append({'role':x[key_name],'content':x[value_name].lstrip('\n')}) # remove leading \n (ToolLLM have these)
        
        to_rotate = x.get('rotate',True) # default to True (only used for adversarial training ,type 3 samples do not rotate)
        ...
        for token_type in range(to_iter):
                ...
                if torch.equal(input_ids[i:i+len(start_ids)], torch.tensor(start_ids)):
                    ...
                    while j <= len(input_ids) - len(end_ids):
                        if torch.equal(input_ids[j:j+len(end_ids)], torch.tensor(end_ids)):
                            spans.append((i, j+len(end_ids))) # span is between start and end
            ...
            if token_type == 0:
                ...
                if to_rotate:
                    ...
                    segment_ids = torch.zeros(len(input_ids), dtype=torch.long) # dont rotate at all
        ...
        batch = {
            ...
            # "labels": labels.tolist() # adding labels seem to incurr a high lost?  ## Disable completion only mask
        }
        if segment_ids is not None:
...
def main(model_family: str, emb_type: str, train_version: str, model_ix: int, run_number: int, hparams: Dict[str, Any]):
    ...
    if 'tool' not in train_dataset_path.lower() and hparams['mode'] == 'sft':
    ...
    if hparams['mode'] == 'sft':
        if 'tool' in train_dataset_path.lower(): # for tooluse, train on ToolACE/ToolLLM
            train_dataset = ToolDataset(train_data, handler, assistant_token=config['assistant_token'], tool_token=config['tool_token'], max_length=max_length,last_completion_only=hparams.get('last_completion_only',False))
            eval_dataset = ToolDataset(eval_data, handler, assistant_token=config['assistant_token'], tool_token=config['tool_token'], max_length=max_length)
        else:
            train_dataset = AlpacaDataset(train_data, template_info, handler, max_length=max_length,combine_system_user=config.get('combine_system_user',False),start_token=config.get('start_token',None),end_token=config.get('end_token',None))
            eval_dataset = AlpacaDataset(eval_data, template_info, handler, max_length=max_length,combine_system_user=config.get('combine_system_user',False),start_token=config.get('start_token',None),end_token=config.get('end_token',None))
    else:
    ...
    if hparams['eval_interval'] > 0: # eval every 20% of total steps
        hparams['evaluation_strategy'] = 'steps'
        hparams['eval_steps'] = max(1, int(total_steps * hparams['eval_interval']))
        hparams['save_strategy'] = 'steps'
        hparams['save_steps'] = max(1, int(total_steps * hparams['eval_interval']))
    ...
    if hparams['mode'] == 'sft':
        ...
        if handler.embedding_type in ['ise','forward_rot']:
            dpo_trainer_class = DPOTrainerWithSegments
            dpo_args['tool_tokens'] = config['tool_token']
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--eval_interval", type=float, default=0., help="eval every percentage of train steps")
