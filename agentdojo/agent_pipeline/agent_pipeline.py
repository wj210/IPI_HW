...
from agentdojo.agent_pipeline.llms.tool_llm import LocalToolLLM
...
def get_llm(provider: str, model: str, config) -> BasePipelineElement:
    if provider == "openai":
        client = openai.OpenAI()
        llm = OpenAILLM(client, model)
    elif 'deepseek' in provider:
        client = openai.OpenAI(base_url="https://api.deepseek.com")
        llm = OpenAILLM(client, model,max_tokens=config.extra_args.get('max_tokens',512))
        ...
        if config.extra_args is None:
            llm = LocalLLM(client, _get_local_model_id(), config=config)
        else:
            llm = LocalLLM(client, None, extra_args=config.extra_args)
    elif provider == "local-tool":
        llm = LocalToolLLM(**config.extra_args)
...
class PipelineConfig(BaseModel):
    ...
    """Whether the local model is a tool-using LLM."""
    max_iters:int = 15
...
class AgentPipeline(BasePipelineElement):
    ...
    def from_config(cls, config: PipelineConfig) -> Self:
        ...
        llm = (
            get_llm(MODEL_PROVIDERS[ModelsEnum(config.llm)], config.llm,config) if isinstance(config.llm, str) else config.llm
        )
        ...
        if config.defense is None:
            tools_loop = ToolsExecutionLoop([ToolsExecutor(tool_output_formatter), llm],max_iters=config.max_iters)
