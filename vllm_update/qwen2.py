...
def generate_isoclinic_rotation_matrix(dim, alpha, device=None, dtype=None): # for ASIDE
    """
...
class Qwen2ModelEmbedding(Qwen2Model):
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = "",
        ...
        self.rotation_matrix = generate_isoclinic_rotation_matrix(
    ...
    def forward(
        ...
        ## rotate if hidden state sequence shape is 1
        if hidden_states.shape[0] == 1: # is a 2D (seq,dim)
    ...
    def gather_segment_ids(self,input_ids): # on the fly gather it 
        mask = torch.zeros_like(input_ids)
        tool_ids = -1
        for curr_start_ids,curr_end_ids in zip(self.start_ids,self.end_ids):
            i = 0
                ...
                if torch.equal(input_ids[i:i+len(curr_start_ids)], curr_start_ids): # find the start of the current start token
                    ...
                    while j <= len(input_ids) - len(curr_end_ids): # look for the end token
                        if torch.equal(input_ids[j:j+len(curr_end_ids)], curr_end_ids):
                            if torch.equal(curr_start_ids, self.start_ids[0]):
                                if tool_ids == -1:
                                    tool_ids = deepcopy(i+len(curr_start_ids))
                                mask[i:j+len(curr_end_ids)] = 1 # for tool, just mark it
                            elif torch.equal(curr_start_ids, self.start_ids[-1]) and i > tool_ids: # if assistant, only mark when the tool has been called
                        ...
                        j += 1
                    if not found:
                        if self.until_last_ids is not None and torch.equal(input_ids[i:i+len(self.until_last_ids)], self.until_last_ids):
