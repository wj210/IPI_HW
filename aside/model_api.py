...
def load_single_emb_model_and_tokenizer(
    ...
    *Note: Original implementation involved having two physical embeddings. This is the only
    ...
    if tokenizer.pad_token is None:
        ...
        print("Added pad_token to tokenizer and resized model embeddings")
    if 'StruQ' in model_cls.__class__.__name__ or is_struq: # for formatting
        tokenizer.is_struq = True
    else:
        tokenizer.is_struq = False
    # print (f'IS STRUQ: {tokenizer.is_struq}')
...
class CustomModelHandler:
    ...
    def _setup_hf_model(self,is_struq=False) -> None:
        ...
        if config_cls is None or model_cls is None:
            raise ValueError(f"Unsupported ({config_cls}, {model_cls})")
...
def format_model_input(
    ...
    chat += suffix # add suffix if needed
...
def format_prompt(prompt, template, role):
    ...
    if role =='user' and isinstance(prompt,dict):
        if len(prompt['input']) < 2:
            prompt['input'] = "No input"
        return template[role].format_map(prompt)
    else:
